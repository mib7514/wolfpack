...
const ASYMMETRIC_TYPES = new Set(["2b", "3a", "3b"]);
...
const ETF_DATA = [
  ...
  { name: "TIME 코스닥액티브", code: "신규", aum: "4,770억", fee: "0.80%", note: "🆕 3/10 상장. 타임폴리오. 2차전지·바이오 중심",
    holdings: [{ name: "에코프로", weight: 9.76 },{ name: "에코프로비엠", weight: 6.89 },{ name: "알지노믹스", weight: 2.2 },{ name: "로보티즈", weight: 1.8 },{ name: "삼천당제약", weight: 1.5 },{ name: "엘앤에프", weight: 1.5 },{ name: "나노신소재", weight: 0.8 },{ name: "테크윙", weight: 0.7 },{ name: "HLB", weight: 0.5 },{ name: "에이비엘바이오", weight: 0.5 }] },
  { name: "KoAct 코스닥액티브", code: "신규", aum: "5,950억", fee: "0.50%", note: "🆕 3/10 상장. 삼성액티브. 공격적 종목 발굴",
    holdings: [{ name: "로보티즈", weight: 3.0 },{ name: "테크윙", weight: 2.0 },{ name: "에코프로비엠", weight: 2.5 },{ name: "에코프로", weight: 2.0 },{ name: "인텔리안테크", weight: 3.0 },{ name: "고영", weight: 1.5 },{ name: "심텍", weight: 1.0 },{ name: "엘앤에프", weight: 1.0 },{ name: "삼천당제약", weight: 0.8 },{ name: "위메이드", weight: 0.5 }] },
  ...
  { name: "KODEX 코스닥150IT", code: "261060", aum: "1,100억", fee: "0.25%", note: "코스닥 IT 섹터. 3번 반도체장비 밀집",
    holdings: [{ name: "테크윙", weight: 3.8 },{ name: "심텍", weight: 1.8 },{ name: "고영", weight: 1.5 },{ name: "인텔리안테크", weight: 1.2 },{ name: "티에스이", weight: 1.0 }] },
  ...
  { name: "PLUS 코스닥150액티브", code: "신규(3/17)", aum: "상장 예정", fee: "0.15%", note: "🔜 3/17 상장 예정. 한화. 최저 보수",
...
function calcETFScores(allCompanies) {
  const asymCompanies = allCompanies.filter(c => ASYMMETRIC_TYPES.has(c.type));
  const asymNames = new Set(asymCompanies.map(c => c.name));
  return ETF_DATA.map((etf) => {
    const matchedHoldings = (etf.holdings || []).filter(h => asymNames.has(h.name)).map(h => {
      const comp = asymCompanies.find(c => c.name === h.name);
      return { ...h, type: comp?.type, detail: comp?.detail };
    });
    const totalExposure = Math.round(matchedHoldings.reduce((s, h) => s + h.weight, 0) * 10) / 10;
    const byType = { "2b": 0, "3a": 0, "3b": 0 };
    ...
    return { ...etf, matchedHoldings, totalExposure, bdeCount: matchedHoldings.length, byType };
  ...
  return (
    <div className="min-h-screen bg-[#0A0E17] text-[#E0E4EC]" style={{fontFamily:"'Noto Sans KR',sans-serif"}}>
      ...
      <div className="px-7 pb-10">
        ...
        {activeTab === "etf" && (<div>
          <div className="bg-[#111827] border border-[#1E2636] rounded-lg p-5 mb-4">
            <div className="text-sm font-bold text-[#4EA8FF] mb-2">ETF 비대칭매력(2b/3) 노출도 매칭</div>
            <div className="text-xs text-[#8892A4] leading-relaxed">150종목 중 <span className="text-[#FFB800] font-bold">비대칭 수익 기회가 큰 2b(메인 미출시)·3a(사이클릭)·3b(구조적)</span>에 실질적으로 몇 %나 노출되는지 기준으로 순위화.</div>
          </div>
          <div className="bg-[#111827] border border-[#1E2636] rounded-lg overflow-hidden">
            ...
            {etfScored.map(etf=>(<div key={etf.code||etf.name} className="grid gap-3 px-4 py-3.5 border-b border-[#1E2636] items-center text-xs hover:bg-[#1A2030] transition" style={{gridTemplateColumns:"2fr 80px 60px 60px 70px 60px 3fr"}}>
              <div><div className="font-bold text-sm">{etf.name}</div><div className="text-[10px] text-[#5A6478] font-mono">{etf.code}</div></div>
              <div className="text-center"><span className={`font-mono text-lg font-black ${etf.totalExposure>=15?"text-[#FFB800]":etf.totalExposure>=5?"text-[#4EA8FF]":"text-[#5A6478]"}`}>{etf.totalExposure}%</span><div className="text-[9px] text-[#5A6478]">{etf.bdeCount}종목</div></div>
              <div className="text-center font-mono text-sm font-bold text-[#FF8C00]">{etf.byType?.["2b"]||0}%</div>
              <div className="text-center font-mono text-sm font-bold text-[#FFB800]">{Math.round(((etf.byType?.["3a"]||0)+(etf.byType?.["3b"]||0))*10)/10}%</div>
              <div className="text-center font-mono text-[#8892A4]">{etf.aum}</div>
              <div className="text-center font-mono text-[#8892A4]">{etf.fee}</div>
              <div><div className="flex flex-wrap gap-1 mb-1">{etf.matchedHoldings.sort((a,b)=>b.weight-a.weight).map(h=>{const dt=DEFICIT_TYPES[h.type]||DEFICIT_TYPES["흑자"];return(<span key={h.name} className="text-[10px] px-1.5 py-0.5 rounded font-mono" style={{background:dt.bg,color:dt.color,border:`1px solid ${dt.color}30`}}>{h.name} <span className="opacity-70">{h.weight}%</span></span>);})}</div><div className="text-[10px] text-[#5A6478]">{etf.note}</div></div>
            </div>))}
          </div>
        ...
        {activeTab === "framework" && (<div>
          ...
          <div className="bg-[#111827] border border-[#FFB80030] rounded-xl p-5 mb-5" style={{borderLeftWidth:3,borderLeftColor:"#FFB800"}}>
            ...
            <div className="text-xs text-[#8892A4] leading-relaxed space-y-3">
              <div><div className="text-[#FFB800] font-bold mb-1">3a (사이클릭 OP적자) — 최고 비대칭매력</div><p>업황 사이클 하강이 주원인. GP마진 건재 + 사이클 회복 시 '자동 흑전'. 별도 구조조정 불필요. 에코프로·테크윙 등 2차전지/반도체 하강기 기업. GP마진 30%+ · 적자폭 분기별 축소 · 사이클 촉매 명확 시 최고.</p></div>
              <div><div className="text-[#E6A800] font-bold mb-1">3b (구조적 투자초과) — 레버리지 기대</div><p>의도적 R&D·마케팅 투자가 GP 초과. 매출 성장률 &gt; 판관비 증가율이면 영업레버리지 전환점. 3a보다 불확실성이 높지만, 플랫폼 기업의 경우 전환 시 폭발력도 큼.</p></div>
              <div><div className="text-[#FF8C00] font-bold mb-1">2b (메인 미출시) — 고위험 고보상 바이너리</div><p>핵심 파이프라인 성공 시 P&L 구조 자체가 바뀜. 복수 파이프라인(옵셔널리티) · 기존매출로 런웨이 확보 · Phase3/NDA 단계일 때 최고.</p></div>
              <div><div className="text-[#94A3B8] font-bold mb-1">4b (일회성 계속사업적자) — 숨은 흑자</div><p>본업 흑자인데 환차손·소송 등 비반복 비용으로 일시 적자. 비용 소멸 시 즉시 흑전. 시장이 '적자'로 할인하는 동안 매수 기회. 단, 3년 연속 '일회성'이면 4a로 격하.</p></div>
            </div>
          </div>
